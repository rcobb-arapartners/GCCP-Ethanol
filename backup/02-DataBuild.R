# 02-DataBuild.R
# Data fetch and prices_full construction.
# All logic is taken verbatim from scratch.R — do not modify.

# ------------------------------------------------------------
# API fetch (used for both ethanol and corn)
# ------------------------------------------------------------
build_marketview_daily_forward_data_on_corn_months <- function(
  ...
  full_data <- full_data_raw |>
    ...
    mutate(
      ...
      MONTH_NUM = case_when(
        month_code == "F" ~ 1L, month_code == "G" ~ 2L, month_code == "H" ~ 3L,
        month_code == "J" ~ 4L, month_code == "K" ~ 5L, month_code == "M" ~ 6L,
        month_code == "N" ~ 7L, month_code == "Q" ~ 8L, month_code == "U" ~ 9L,
        month_code == "V" ~ 10L, month_code == "X" ~ 11L, month_code == "Z" ~ 12L,
        TRUE ~ NA_integer_
      ),
...
# ------------------------------------------------------------
# OFF corn months (verbatim from scratch.R)
# ------------------------------------------------------------
generate_off_month_prices <- function(marketview_ethanol_df,
                                      marketview_corn_df,
                                      as_of_date = '2026-02-06',
                                      yield = 2.9) {
  ...
  # expired contracts
  ethanol_off_df_expired <- off_month_ethanol_prices |> filter(EXPIRED == 1)
  full_relevant_corn_off_prices_expired <- data.frame()
  
  for(d in unique(ethanol_off_df_expired$DESCRIPTION)) {
    sub_ethanol_off_df_expired <- ethanol_off_df_expired |> filter(DESCRIPTION == d)
    ethanol_year_code  <- sub_ethanol_off_df_expired |> pull(YEAR_CODE)
    ethanol_month_code <- sub_ethanol_off_df_expired |> pull(MONTH_CODE)
    marketview_corn_df_filtered <- marketview_corn_df |> filter(DATE_ID <= as_of_date)
    
    if(ethanol_month_code == 'F') {
      sub_marketview_corn_expired_df_filtered <- marketview_corn_df_filtered |>
        filter(MONTH_CODE == 'H', YEAR_CODE == ethanol_year_code) |>
        filter(months(TRADING_MONTH) == 'January') |>
        filter(TRADING_MONTH == max(TRADING_MONTH)) |>
        mutate(RELEVANT_CORN_PRICE = mean(CLOSE,na.rm=T), ETHANOL_DESCRIPTION = d) |>
        select(ETHANOL_DESCRIPTION,RELEVANT_CORN_PRICE) |> distinct()
    }
    ...
    if(ethanol_month_code == 'H') {
      sub_marketview_corn_expired_df_filtered <- marketview_corn_df_filtered |>
        filter(MONTH_CODE == 'H', YEAR_CODE == ethanol_year_code) |>
        filter(months(TRADING_MONTH) == 'February') |>
        mutate(RELEVANT_CORN_PRICE = last(CLOSE), ETHANOL_DESCRIPTION = d) |>
        select(ETHANOL_DESCRIPTION,RELEVANT_CORN_PRICE) |> distinct()
    }
    if(ethanol_month_code == 'J') {
      sub_marketview_corn_expired_df_filtered <- marketview_corn_df_filtered |>
        filter(MONTH_CODE == 'K', YEAR_CODE == ethanol_year_code) |>
    ...
    if(ethanol_month_code == 'M') {
      sub_marketview_corn_expired_df_filtered <- marketview_corn_df_filtered |>
        filter(MONTH_CODE == 'N', YEAR_CODE == ethanol_year_code) |>
    ...
    if(ethanol_month_code == 'Q') {
      sub_marketview_corn_expired_df_filtered <- marketview_corn_df_filtered |>
        filter(MONTH_CODE == 'U', YEAR_CODE == ethanol_year_code) |>
    ...
    if(ethanol_month_code == 'V') {
      sub_marketview_corn_expired_df_filtered <- marketview_corn_df_filtered |>
        filter(MONTH_CODE == 'Z', YEAR_CODE == ethanol_year_code) |>
  ...
  ethanol_off_df_non_expired <- off_month_ethanol_prices |> filter(EXPIRED == 0)
  full_relevant_corn_off_prices_non_expired <- data.frame()
  
  for(d in unique(ethanol_off_df_non_expired$DESCRIPTION)) {
    sub_ethanol_off_df_non_expired <- ethanol_off_df_non_expired |> filter(DESCRIPTION == d)
    ethanol_year_code  <- sub_ethanol_off_df_non_expired |> pull(YEAR_CODE)
    ethanol_month_code <- sub_ethanol_off_df_non_expired |> pull(MONTH_CODE)
    ...
    if(ethanol_month_code == 'F') {
      sub_marketview_corn_non_expired_df_filtered <- marketview_corn_df_filtered |>
        filter(MONTH_CODE == 'H') |>
        mutate(ETHANOL_DESCRIPTION = d) |> select(ETHANOL_DESCRIPTION, RELEVANT_CORN_PRICE = CLOSE)
    }
    if(ethanol_month_code == 'G') {
      sub_marketview_corn_non_expired_df_filtered <- marketview_corn_df_filtered |>
        filter(MONTH_CODE == 'H') |>
...
# ------------------------------------------------------------
# ON corn months (verbatim from scratch.R)
# ------------------------------------------------------------
generate_on_month_prices <- function(input_ethanol_df,
                                     input_corn_df,
                                     as_of_date = '2026-02-06',
                                     yield = 2.9) {
  ...
  ethanol_df <- input_ethanol_df |>
    ...
    mutate(
      ...
      CONTRACT_MONTH_NAME = case_when(
        CONTRACT_MONTH == "F" ~ "January",   CONTRACT_MONTH == "G" ~ "February",
        CONTRACT_MONTH == "H" ~ "March",     CONTRACT_MONTH == "J" ~ "April",
        CONTRACT_MONTH == "K" ~ "May",       CONTRACT_MONTH == "M" ~ "June",
        CONTRACT_MONTH == "N" ~ "July",      CONTRACT_MONTH == "Q" ~ "August",
        CONTRACT_MONTH == "U" ~ "September", CONTRACT_MONTH == "V" ~ "October",
        CONTRACT_MONTH == "X" ~ "November",  CONTRACT_MONTH == "Z" ~ "December",
        TRUE ~ NA_character_
      ),
      RELEVANT_CORN_CONTRACT_MONTH = case_when(
        CONTRACT_MONTH == "Z" ~ "H", CONTRACT_MONTH == "X" ~ "Z",
        CONTRACT_MONTH == "V" ~ "Z", CONTRACT_MONTH == "U" ~ "Z",
        CONTRACT_MONTH == "Q" ~ "U", CONTRACT_MONTH == "N" ~ "U",
        CONTRACT_MONTH == "M" ~ "N", CONTRACT_MONTH == "K" ~ "N",
        CONTRACT_MONTH == "J" ~ "K", CONTRACT_MONTH == "H" ~ "K",
        CONTRACT_MONTH == "G" ~ "H", CONTRACT_MONTH == "F" ~ "H",
        TRUE ~ NA_character_
      ),
      RELEVANT_CORN_CONTRACT_YEAR =
        if_else(CONTRACT_MONTH == "Z", CONTRACT_YEAR + 1L, CONTRACT_YEAR),
      ETHANOL_MONTH_NUM = case_when(
        CONTRACT_MONTH == "F" ~ 1L,  CONTRACT_MONTH == "G" ~ 2L,
        CONTRACT_MONTH == "H" ~ 3L,  CONTRACT_MONTH == "J" ~ 4L,
        CONTRACT_MONTH == "K" ~ 5L,  CONTRACT_MONTH == "M" ~ 6L,
        CONTRACT_MONTH == "N" ~ 7L,  CONTRACT_MONTH == "Q" ~ 8L,
        CONTRACT_MONTH == "U" ~ 9L,  CONTRACT_MONTH == "V" ~ 10L,
        CONTRACT_MONTH == "X" ~ 11L, CONTRACT_MONTH == "Z" ~ 12L,
        TRUE ~ NA_integer_
      ),
  ...
  corn_join <- input_corn_df |>
    rename(CONTRACT_YEAR = CALENDAR_YEAR) |>
    select(DATE_ID, CONTRACT_YEAR, MONTH_CODE,
           CORN_DESCRIPTION = DESCRIPTION, CORN_CLOSE = CLOSE)
  
  on_corn_months_crush_df <- ethanol_df |>
    left_join(
      corn_join,
      by = c("DATE_ID"                       = "DATE_ID",
             "RELEVANT_CORN_CONTRACT_YEAR"   = "CONTRACT_YEAR",
             "RELEVANT_CORN_CONTRACT_MONTH"  = "MONTH_CODE")
    ) |>
    rename(ETHANOL_DESCRIPTION = DESCRIPTION, ETHANOL_CLOSE = CLOSE) |>
    select(DATE_ID, CONTRACT_YEAR, CONTRACT_MONTH, CONTRACT_MONTH_NAME,
           RELEVANT_CORN_CONTRACT_YEAR, RELEVANT_CORN_CONTRACT_MONTH,
           ETHANOL_DESCRIPTION, CORN_DESCRIPTION, ETHANOL_EXPIRY_CEILING_DATE,
           EXPIRED_CONTRACT, ETHANOL_CLOSE, CORN_CLOSE) |>
    na.omit() |>
...
# ------------------------------------------------------------
# Build prices_full from raw API dataframes
# ------------------------------------------------------------
build_prices_full <- function(ethanol_df, corn_df, as_of_date, yield = 2.9) {
  off <- generate_off_month_prices(ethanol_df, corn_df, as_of_date, yield)
  on  <- generate_on_month_prices(ethanol_df, corn_df, as_of_date, yield)
  off |> full_join(on, by = "DESCRIPTION")
}

# ------------------------------------------------------------
# Shape prices_full into a year x month gt-ready wide table
# ------------------------------------------------------------
shape_for_table <- function(prices_full, spread_col) {
  month_order <- c("January","February","March","April","May","June",
                   "July","August","September","October","November","December")
  
  prices_full |>
    mutate(
      YEAR = as.integer(stringr::str_extract(DESCRIPTION, "\\d{2}(?= NYMEX)")) + 2000L,
      MONTH_NAME = case_when(
        grepl("^Jan", DESCRIPTION) ~ "January",   grepl("^Feb", DESCRIPTION) ~ "February",
        grepl("^Mar", DESCRIPTION) ~ "March",      grepl("^Apr", DESCRIPTION) ~ "April",
        grepl("^May", DESCRIPTION) ~ "May",        grepl("^Jun", DESCRIPTION) ~ "June",
        grepl("^Jul", DESCRIPTION) ~ "July",       grepl("^Aug", DESCRIPTION) ~ "August",
        grepl("^Sep", DESCRIPTION) ~ "September",  grepl("^Oct", DESCRIPTION) ~ "October",
        grepl("^Nov", DESCRIPTION) ~ "November",   grepl("^Dec", DESCRIPTION) ~ "December",
        TRUE ~ NA_character_
      )
    ) |>
